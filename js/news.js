(function () {
  ...
  const fallbackItems = [
    {
      ...
      cn: { title: "梅亞嵐參加第三屆中華建設管理研究會建築安全學術研討會", summary: "博士研究生梅亞嵐在清華大學舉行的研究生論壇作匯報，並參與圓桌論壇提問交流。", url: "news/articles/2026-09-11-mei-yalan-safety-seminar_cn.html" },
  ...
  function localized(item, language) {
    return item[language] && item[language].title ? item[language] : item.en;
  }

  function card(item, language, root) {
    const text = localized(item, language);
    const words = labels[language] || labels.en;
    const date = new Intl.DateTimeFormat(words.locale, { year: "numeric", month: "short", day: "numeric", timeZone: "UTC" }).format(new Date(item.date + "T00:00:00Z"));
    const url = joinPath(root, text.url);
    return '<article class="news-card" data-news-type="' + escapeHtml(item.type) + '">' +
      '<a class="news-card__image-link" href="' + escapeHtml(url) + '" aria-label="' + escapeHtml(text.title) + '">' +
      '<img class="news-card__image" src="' + escapeHtml(joinPath(root, item.cover)) + '" alt="" loading="lazy"></a>' +
      '<div class="news-card__body"><div class="news-card__meta"><span class="news-type">' + escapeHtml(words[item.type] || words.other) + '</span><time datetime="' + escapeHtml(item.date) + '">' + escapeHtml(date) + '</time></div>' +
      '<h2 class="news-card__title"><a href="' + escapeHtml(url) + '">' + escapeHtml(text.title) + '</a></h2>' +
      '<p class="news-card__summary">' + escapeHtml(text.summary) + '</p><a class="news-card__more" href="' + escapeHtml(url) + '">' + escapeHtml(words.more) + ' &rarr;</a></div></article>';
  }
  ...
  function render(container, items) {
    ...
    let selected = items.slice().sort(function (a, b) { return b.date.localeCompare(a.date); });
    if (container.dataset.featured === "true") selected = selected.filter(function (item) { return item.featured; });
    if (limit) selected = selected.slice(0, limit);
    container.innerHTML = selected.map(function (item) { return card(item, language, root); }).join("");
  }
  ...
  function setupFilters(container, items) {
    ...
    filters.addEventListener("click", function (event) {
      ...
      if (!button) return;
      filters.querySelectorAll("[data-filter]").forEach(function (item) { item.classList.toggle("is-active", item === button); });
      const type = button.dataset.filter;
      const filtered = type === "all" ? items : items.filter(function (item) { return item.type === type; });
      render(container, filtered);
    });
  }

  document.querySelectorAll("[data-news-grid]").forEach(function (container) {
    const root = container.dataset.root || ".";
    fetch(joinPath(root, "news/news.json"))
      .then(function (response) { if (!response.ok) throw new Error("News data unavailable"); return response.json(); })
      .catch(function () { return fallbackItems; })
      .then(function (items) { render(container, items); setupFilters(container, items); });
  });
}());
